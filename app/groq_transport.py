...
class GroqChatTransport:
    def __init__(
        ...
        api_key: str,
        model: str,
        http_post: Callable[..., Any] = stdlib_http_post,
    ):
        if not api_key:
    ...
    def _payload(
        self,
        request: ExternalSynthesisRequest,
    ) -> dict[str, Any]:
        evidence = [
            {
    ...
    def synthesize(
        ...
        response = self._http_post(
            ...
            headers={
                ...
                "User-Agent": "north-star-groq-uat/1.0",
            },
        ...
        if status != 200:
            ...
            if status in (401, 403):
                ...
                if provider_message_class:
                    reason = (
                        f"{reason}_msg_{provider_message_class}"
                    )
            ...
            if response_format:
                reason = (
                    f"{reason}_format_{response_format}"
                )
        ...
        try:
            ...
            raise ExternalLLMInvalidResponse(
